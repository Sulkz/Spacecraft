...
public class Light {
  ...
  public Light(GL3 gl) {
    ...
    direction = new Vec3(-1f, -1f, 0f); // Default downward spotlight
    cutOff = (float) Math.cos(Math.toRadians(14.5f)); // Inner cutoff angle
    outerCutOff = (float) Math.cos(Math.toRadians(20.5f)); // Outer cutoff angle
    model = new Mat4(1);
  ...
  public void setCutOff(float angle) {
    cutOff = (float) Math.cos(Math.toRadians(angle));
  }
  ...
  public void setOuterCutOff(float angle) {
    outerCutOff = (float) Math.cos(Math.toRadians(angle));
  }
